...
describe('ssl_handler', function()
  ...
  describe('.handle', function()
    ...
    context('when ssl certs are fetched properly', function()
      ...
      it ('caches the DER format of the cert', function()
        ...
        assert.is_nil(err)
        assert.are.equal(cache:get('www.foobar.com:dcrt'), crt_der)
        assert.are.equal(cache:get('www.foobar.com:dkey'), key_der)
      end)
    end)
    ...
    context('certs are cached', function()
      ...
      it('fetches the certs from the cache', function()
        cache:set('foo-bar-express.rise.cloud:dcrt', 'DER-CRT')
        cache:set('foo-bar-express.rise.cloud:dkey', 'DER-KEY')
        ...
        assert.are.equal(cache:get('foo-bar-express.rise.cloud:dcrt'), 'DER-CRT')
        assert.are.equal(cache:get('foo-bar-express.rise.cloud:dkey'), 'DER-KEY')
      end)
    end)
  ...
  describe(".invalidate_cache", function()
    it("invalidates prefix cache for given host name", function()
      cache:set("foo-bar-express.rise.cloud:dcrt", "crt-to-be-gone")
      cache:set("foo-bar-express.rise.cloud:dkey", "key-to-be-gone")
      cache:set("foo-bar-express.com:dcrt", "crt-to-be-here")
      cache:set("foo-bar-express.com:dkey", "key-to-be-here")

      ssl_handler.invalidate_cache("foo-bar-express.rise.cloud")

      assert.is_nil(cache:get("foo-bar-express.rise.cloud:dcrt"))
      assert.is_nil(cache:get("foo-bar-express.rise.cloud:dkey"))
      assert.are.equal(cache:get("foo-bar-express.com:dcrt"), "crt-to-be-here")
      assert.are.equal(cache:get("foo-bar-express.com:dkey"), "key-to-be-here")
    end)
  end)
end)
