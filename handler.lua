...
function handler.handle(host, path) -- returns (target, err, err_log)
  ...
  if not prefix then
    ...
    if err then
      return nil, handler.err_not_found, "Failed to fetch metadata for "..host.." due to "..err
    end

    if not meta.prefix then
      return nil, handler.internal_server_error, "meta.json for "..host.." did not contain prefix"
    end
  ...
  if not target_path or should_redirect == nil then -- should_redirect is boolean
    ...
    if err or not target_path then
      return nil, handler.internal_server_error, "could not resolve "..host..path
    end
  ...
  if should_redirect then
    return target_path, handler.err_redirect, nil
  end
